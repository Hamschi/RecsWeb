...
def generate_html(data: dict, instructions: list[str]) -> str:
	...
	for ing in data.get("ingredients", []):
		...
		if not has_amount:
			li = ('<li class="list-group-item d-flex align-items-center gap-2">'
...
<div class=\"container\">
	...
	<div class=\"col\">
		...
		Quelle: <a href=\"{source_href}\" class=\"link-light link-underline-opacity-0 link-underline-opacity-75-hover\">{source_text}</a>
		</div>
	</div>
	</div>
</div>
...
class RecipeApp(tk.Tk):
	...
	def build_form(self):
		...
		row += 1
		ttk.Label(self.inner, text="Originalität (0–5, Default 5)").grid(row=row, column=0, sticky="w", **pad)
		...
		# Source (Quelle)
		row += 1
		ttk.Label(self.inner, text="Quelle").grid(row=row, column=0, sticky="w", **pad)
